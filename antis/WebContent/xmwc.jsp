...
<nav class="navbar navbar-inverse navbar-fixed-top" role="navigation" id="navnewclass">
	...
	<table class="table table-hover" style="width:1176px;">
		...
		<tr>
			...
			<td>${fpxm.xzrdw  }</td>
			<td>${fpxm.xtzze }</td>
			<td><a href="${pageContext.request.contextPath}/fpxm/xmwc2.action?id=${fpxm.xid}">项目完成</a>
							</td>
		</tr>
		</c:forEach>
	</table>
	...
	<a href="${pageContext.request.contextPath}/fpxm/xmwc.action?pageNow=1">首页</a>
	<c:choose>
		<c:when test="${page.pageNow - 1 > 0}">
			<a href="${pageContext.request.contextPath}/fpxm/xmwc.action?pageNow=${page.pageNow - 1}">上一页</a>
		</c:when>
		<c:when test="${page.pageNow - 1 <= 0}">
			<a href="${pageContext.request.contextPath}/fpxm/xmwc.action?pageNow=1">上一页</a>
		</c:when>
	</c:choose>
	<c:choose>
		<c:when test="${page.pageNow + 1 < page.totalPageCount}">
			<a href="${pageContext.request.contextPath}/fpxm/xmwc.action?pageNow=${page.pageNow + 1}">下一页</a>
		</c:when>
		<c:when test="${page.pageNow + 1 >= page.totalPageCount}">
			<a href="${pageContext.request.contextPath}/fpxm/xmwc.action?pageNow=${page.totalPageCount}">下一页</a>
		</c:when>
	</c:choose>
	<a href="${pageContext.request.contextPath}/fpxm/xmwc.action?pageNow=${page.totalPageCount}">尾页</a>
	</center>
<!-- 	<nav aria-label="...">
