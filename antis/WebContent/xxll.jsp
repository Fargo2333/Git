...
<form name="mbForm" action="${pageContext.request.contextPath}/mb/querymb.action" method="post">
 <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation" id="navnewclass">
				...
				<table class="table table-hover">
					...
					<c:forEach items="${MbList}" var="mb">
						<tr align="center" id="fptableheight">
							...
							<td style="width:25%">
							&nbsp;&nbsp;
							    <a href="${pageContext.request.contextPath}/mb/xxll2.action?id=${mb.mid}">浏览</a>
							</td>
						</tr>
					</c:forEach>
				</table>
	...
	<a href="${pageContext.request.contextPath}/mb/xxll.action?pageNow=1">首页</a>
	<c:choose>
		<c:when test="${page.pageNow - 1 > 0}">
			<a href="${pageContext.request.contextPath}/mb/xxll.action?pageNow=${page.pageNow - 1}">上一页</a>
		</c:when>
		<c:when test="${page.pageNow - 1 <= 0}">
			<a href="${pageContext.request.contextPath}/mb/xxll.action?pageNow=1">上一页</a>
		</c:when>
	</c:choose>
	<c:choose>
		<c:when test="${page.pageNow + 1 < page.totalPageCount}">
			<a href="${pageContext.request.contextPath}/mb/xxll.action?pageNow=${page.pageNow + 1}">下一页</a>
		</c:when>
		<c:when test="${page.pageNow + 1 >= page.totalPageCount}">
			<a href="${pageContext.request.contextPath}/mb/xxll.action?pageNow=${page.totalPageCount}">下一页</a>
		</c:when>
	</c:choose>
	<a href="${pageContext.request.contextPath}/mb/xxll.action?pageNow=${page.totalPageCount}">尾页</a>
	</center>
				</div>
				</div>
			</td>
