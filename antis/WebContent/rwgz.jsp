...
<form name="mbForm" action="${pageContext.request.contextPath}/mb/querymb.action" method="post">
 <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation" id="navnewclass">
				...
				<table class="table table-hover">
					...
					<c:forEach items="${BftjList}" var="bftj">
						<tr align="center" >
							...
							<td style="width:25%">
							&nbsp;&nbsp;
							    <a href="${pageContext.request.contextPath}/bftj/rwgz2.action?id=${bftj.tid}">设置目前计划完成进度</a>
							</td>
						</tr>
					</c:forEach>
				</table>
	...
	<a href="${pageContext.request.contextPath}/bftj/rwgz.action?pageNow=1">首页</a>
	<c:choose>
		<c:when test="${page.pageNow - 1 > 0}">
			<a href="${pageContext.request.contextPath}/bftj/rwgz.action?pageNow=${page.pageNow - 1}">上一页</a>
		</c:when>
		<c:when test="${page.pageNow - 1 <= 0}">
			<a href="${pageContext.request.contextPath}/bftj/rwgz.action?pageNow=1">上一页</a>
		</c:when>
	</c:choose>
	<c:choose>
		<c:when test="${page.pageNow + 1 < page.totalPageCount}">
			<a href="${pageContext.request.contextPath}/bftj/rwgz.action?pageNow=${page.pageNow + 1}">下一页</a>
		</c:when>
		<c:when test="${page.pageNow + 1 >= page.totalPageCount}">
			<a href="${pageContext.request.contextPath}/bftj/rwgz.action?pageNow=${page.totalPageCount}">下一页</a>
		</c:when>
	</c:choose>
	<a href="${pageContext.request.contextPath}/bftj/rwgz.action?pageNow=${page.totalPageCount}">尾页</a>
	</center>
				</div>
				</div>
			</td>
