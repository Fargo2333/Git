...
<form name="mbForm" action="${pageContext.request.contextPath}/mb/querymb.action" method="post">
 <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation" id="navnewclass">
		...
		<div class="row">
			...
			<td>
			 ...
			 <div class="panel panel-default">
			  <div class="panel-heading"> 扶贫措施</div>
				<table class="table table-hover">
					...
					<c:forEach items="${PkhList}" var="pkh">
						<tr align="center" >
							...
							<td>
							&nbsp;&nbsp;
							    <a href="${pageContext.request.contextPath}/bftj/zdfpcs.action?id=${pkh.pid}">制定扶贫措施</a>
							</td>
						</tr>
					</c:forEach>
				</table>
	...
	<a href="${pageContext.request.contextPath}/bftj/fpcs.action?pageNow=1">首页</a>
	<c:choose>
		<c:when test="${page.pageNow - 1 > 0}">
			<a href="${pageContext.request.contextPath}/bftj/fpcs.action?pageNow=${page.pageNow - 1}">上一页</a>
		</c:when>
		<c:when test="${page.pageNow - 1 <= 0}">
			<a href="${pageContext.request.contextPath}/bftj/fpcs.action?pageNow=1">上一页</a>
		</c:when>
	</c:choose>
	<c:choose>
		<c:when test="${page.pageNow + 1 < page.totalPageCount}">
			<a href="${pageContext.request.contextPath}/bftj/fpcs.action?pageNow=${page.pageNow + 1}">下一页</a>
		</c:when>
		<c:when test="${page.pageNow + 1 >= page.totalPageCount}">
			<a href="${pageContext.request.contextPath}/bftj/fpcs.action?pageNow=${page.totalPageCount}">下一页</a>
		</c:when>
	</c:choose>
	<a href="${pageContext.request.contextPath}/bftj/fpcs.action?pageNow=${page.totalPageCount}">尾页</a>
	</center>
				</div>
				</div>
			</td>
